	...
	render : function() {
		...
		if (this.props.errors){
			...
			for(var i = 0; i < this.props.errors.length; i++) {
				...
				this.props.errors[i].props.key = i;
				this.props.errors[i].props.className = "errors__p"
		...
		return (
			<div className="form__item">
				<div className="form__input-container">
					...
					<input tabIndex="1" className={class_arr.join(' ')} type="text" value={this.props.period} onChange={this.handleChange} />
				</div>
		...
		return (
			<div className="form__item">
				<div className="form__input-container">
					...
					<input tabIndex="2" className={class_arr.join(' ')} type="text" value={this.props.income} onChange={this.handleChange} />
				</div>
				<div className="form__errors">
				{errors}
				</div>
	...
	render: function() {
		...
		return (
			...
			<div className="form__input-container">
					...
					<input tabIndex="-1" className={class_arr.join(' ')} type="text" defaultValue={this.props.patent_cost} value={this.props.patent_cost} readOnly />
			</div>
			</div>
		);
	}
...
var PersonalIncomeTax = React.createClass({
	render: function() {
		...
		return (
			...
			<input tabIndex="-1" className={class_arr.join(' ')} type="text" defaultValue={this.props.amount} value={this.props.amount} readOnly/>
			</div>
			</div>
		);
	}
...
var IncomeTaxed = React.createClass({
	...
	render: function() {
		...
		return (
			...
			<input tabIndex="3" className={class_arr.join(' ')} type="text" defaultValue={this.props.amount} value={this.props.amount} onChange={this.handleChange} />
			</div>
			<div className="form__errors">
				{errors}
			</div>
...
var MandatoryPC = React.createClass({
	...
	render: function() {
		...
		return (
			...
			<input tabIndex="4" className={class_arr.join(' ')} type="text" defaultValue={this.props.amount} value={this.props.amount} onChange = {this.handleChange} />
			</div>
			<div className="form__errors">
				{errors}
			</div>
...
var ExplainDiv = React.createClass({
	...
	render : function() {
		...
		return <div className="form_explain-div" onMouseLeave={ this.hide_explain } style={this.css_style}>
		<p className="explain-div__p" dangerouslySetInnerHTML={this.props.explain_info.explain_html}></p>
		</div>;
	}
})
...
var PatentForm = React.createClass({
	...
	render: function() {
		...
		for (var name in this.state ) {
			...
			if (isNaN(local_nmb)) {
				errmsg[name] = [<p>Не является числом</p>];
		...
		if (errmsg.income === undefined 
			...
			if (local_state.income > max_period_income ) {
				errmsg['income'] = [<p>Ваш доход за указанный период не может превышать <a className="link-hint" href="#" 
		...
		if (errmsg.income_taxed === undefined 
			...
			if (local_state.income_taxed < min_income_taxed || local_state.income_taxed > local_state.income) {
				errmsg['income_taxed'] = [
					<p>Заявленный доход для исчисления обязательных пенсионных отчислений за указанный период не может быть меньше чем <a className="link-hint" href="#"
		...
		if (errmsg.mandatory_pc === undefined 
			...
			if (local_state.mandatory_pc < min_mandatory_pc || local_state.mandatory_pc > min_mandatory_pc) {
				errmsg['mandatory_pc'] = [
					<p>Обязательные пенсионные очисления за указаный период не могут быть меньше или превышать сумму в <a className="link-hint" href="#"
				...
				errmsg['mandatory_pc'] = [
					<p>Обязательные пенсионные очисления за указаный период не могут превышать <a className="link-hint" href="#"
		...
		if (errmsg.mandatory_pc === undefined 
			...
			temp_social_tax = Math.round(local_state.patent_cost * 0.5);
